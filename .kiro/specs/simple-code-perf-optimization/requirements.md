...
4. THE 桶内名次为 i（按 `cmp_in_bucket` 排序，0 起）的退火出简字 SHALL 使用「可用偏好表」中第 `i mod K'` 个上屏键；其中「可用偏好表」为该级偏好表去除被本桶固定简码占用的上屏键后的剩余序列，`K'` 为其长度。
...
4. WHEN 过滤后某桶的可用上屏键序列为 `K'` 项（异手字母数 + 含 `_` ? 1），THE 桶内名次 i 的退火出简字 SHALL 使用该可用序列中第 `i mod K'` 个上屏键（在需求 36 偏好表顺序的基础上去除同手字母后，再去除固定简码已占用的上屏键）。
...
7. WHEN 简码评估器为某级别的某个核心简码桶选取退火出简候选，THE 简码评估器 SHALL 仅分配 `max(0, code_num − 该桶固定占用数)` 个优化简码，且退火字使用的上屏键为「去除该桶固定简码已占用上屏键后的可用偏好表」（需求 36.4）；固定占用数本身不受 `code_num` 限制（固定简码为权威预分配，可达到或超过 `code_num`），且固定简码不因占用达到/超过 `code_num` 而被拒绝。
