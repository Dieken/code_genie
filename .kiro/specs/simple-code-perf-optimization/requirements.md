...
- **有效简码（Effective_Simple_Code）**：一个完整的、可独立上屏的简码单位。WHERE 该级 `commit_keys` 为空，有效简码 = 核心简码（核心键位串本身）；WHERE 该级 `commit_keys` 非空，有效简码 = 核心简码 + 一个上屏键（故同一核心简码桶含 K 个互不相同的有效简码，K = 该级不同上屏键数）。
- **`code_num`（每有效简码字数上限）**：一个**有效简码**最多可对应（分配给）的全码汉字数。例：`commit_keys="dk"`、`code_num=1` 时，核心简码 `A` 派生的有效简码 `Ad` 至多对应 1 个全码字、`Ak` 也至多对应 1 个；`code_num=2` 时 `Ad` 至多 2 个、`Ak` 至多 2 个，二者各自独立、均不得超过 `code_num`。`code_num` 并非「该级别可分配的简码总数」。`code_num == 0` 表示该级退火不分配任何简码（固定简码仍生效，见需求 21.13）。
- **简码编码（Simple_Code）**：某汉字在某简码级别下按规则计算得到的编码值（整数），仅由核心简码决定（上屏键不进入编码）。
- **简码桶（Simple_Bucket）**：同一简码级别中映射到同一简码编码（即同一核心简码）的候选字集合；一个桶含 `max(K,1)` 个有效简码（无上屏键级别为 1 个）。
...
- **上屏键序列（Commit_Keys）**：某简码级别的字符串属性，按优先级列出该级简码的候选上屏键（字母为主键盘键、`_` 为空格键），空串表示自动上屏（无额外上屏键）。取代历史布尔属性 `space_commit`（`""`≡false、`"_"`≡true）；在 output 文件中以核心简码后追加实际上屏键字符表示。
- **固定简码（Fixed_Simple_Code）**：由配置预先定义的「汉字 → 简码」映射，简码为完整字面键位串（末键可为任意上屏键或无）；固定简码的汉字不参与退火的简码分配，可归属某级或无归属（仍输出）。
...
### 需求 20：简码级别的上屏键序列配置（取代「空格上屏」布尔）

**用户故事：** 作为方案设计者，我希望每个简码级别能指定一个「上屏键序列」（而不仅仅是「是否空格上屏」），从而让同一核心简码借助不同上屏键扩展出多个互不冲突的完整编码、降低重码，并让码长/当量/分布/输出反映真实击键成本。

#### 术语

- **上屏键序列（commit_keys）**：字符串配置项，列出该级简码可用的上屏键，按优先级排序。字母表示主键盘区按键，下划线 `_` 表示空格键（`KEY_SPACE`）。空串表示「自动上屏」（无需额外上屏键）。
- **核心简码（core code）**：由该级 `rule` 对逻辑根编码得到的键位串，不含上屏键。**桶编码仍只由核心简码决定**（与 `calc_simple_code` 一致），上屏键不进入桶编码。
- **K**：该级不同上屏键数 = `|左手字母| + |右手字母| + (commit_keys 含 _ ? 1 : 0)`。

#### 验收标准

1. THE 优化器 SHALL 在每个 `[[simple_levels]]` 级别配置下提供字符串配置项 `commit_keys`，作为该级上屏键的优先级序列。
2. WHEN 某简码级别配置缺失 `commit_keys`，THE 优化器 SHALL 采用默认值空串 `""`（自动上屏）。
3. THE 下划线 `_`（空格上屏键）SHALL 只允许出现在 `commit_keys` 的开头或末尾；IF `_` 出现在中间位置，THEN THE 优化器 SHALL 在解析配置时报错。
4. WHERE 某级 `commit_keys` 为空串，THE 简码评估器 SHALL 使该级简码不追加任何上屏键：有效码长 = 核心码长，加权当量与分布偏差均不含上屏键，输出不追加上屏字符；该行为 SHALL 与历史布尔配置 `space_commit = false` 逐字段一致。
5. WHERE 某级 `commit_keys` 非空，THE 简码评估器 SHALL 使该级每个出简字在核心简码后追加恰好一个上屏键，使有效码长 = 核心码长 + 1（与具体上屏键无关）；该 +1 SHALL 作为退火前已知确定值预计算到 `simple_base_saving`，且效率排序键据此取 `base_saving = full_len - (核心码长 + 1)`。
6. WHERE 某级 `commit_keys` 恰为 `"_"`，THE 该级行为 SHALL 与历史布尔配置 `space_commit = true` 逐字段一致（每个出简字以空格上屏；当量计入末位键到 `KEY_SPACE` 的转移，分布对 `key_usage[KEY_SPACE]` 与 `key_presses` 各计一次，输出追加 `_`）。
7. WHERE 某出简字实际分得上屏键 `c`（由需求 36 决定），THE 简码评估器 SHALL 在其有效击键序列末尾计入键 `c`，使该键计入加权当量（末位核心键到 `c` 的转移当量）与分布偏差（`key_usage[c]` 与 `key_presses` 各计一次）。
8. WHEN 优化器将某出简字的简码输出到 output 文件，THE 优化器 SHALL 在核心简码字符串后追加该字实际分得上屏键对应的字符（字母键追加该字母、空格键追加 `_`），例如核心简码 `ab` 用上屏键 `d` 输出为 `abd`、用空格输出为 `ab_`。
9. THE 字频模式排序键（以 `freq` 为键）SHALL 不受上屏键影响。
10. THE 优化器 SHALL 接受历史布尔配置项 `space_commit` 作为已废弃的兼容输入：WHEN 某级配置了 `space_commit` 而未配置 `commit_keys`，THE 优化器 SHALL 输出废弃告警并将 `space_commit = true` 映射为 `commit_keys = "_"`、`space_commit = false` 映射为 `commit_keys = ""`；WHEN 同时配置了 `commit_keys` 与 `space_commit`，THE 优化器 SHALL 以 `commit_keys` 为准并告警忽略 `space_commit`。

### 需求 36：上屏键的左右手交替偏好与桶内分配

**用户故事：** 作为方案设计者，我希望出简字优先使用与核心末键不同手的上屏键（提升左右手交替、改善手感），并在同一核心简码对应多个汉字时按确定规则分配上屏键，从而让分配可复现、且与退火增量评估一致。

#### 术语

- **手别（hand）**：按标准 QWERTY 物理布局，主键盘区 30 键分为左手（`q w e r t a s d f g z x c v b`）与右手（`y u i o p h j k l ; n m , . /`）；空格键 `_` 为中性键，不属任何手。
- **偏好表（preference table）**：该级一个长度为 K 的上屏键有序序列，决定桶内名次 i 的出简字使用哪个上屏键。

#### 验收标准

1. THE 优化器 SHALL 按 QWERTY 物理布局将 `commit_keys` 中的字母分为左手子序列与右手子序列，各子序列内保持其在 `commit_keys` 中的原相对顺序；`_`（若有）不参与左右手划分。
2. THE 优化器 SHALL 为每级预计算两张偏好表（分别对应「核心末键属左手」与「核心末键属右手」），每张表的构成为：先取「核心末键对侧手」的字母子序列、再取「核心末键同侧手」的字母子序列（各自保序），再按 `_` 在 `commit_keys` 中的位置将其加在该表的开头（`_` 在 `commit_keys` 开头时）或末尾（`_` 在 `commit_keys` 末尾时）。
3. WHEN 简码评估器为某核心简码桶选取出简字，THE 简码评估器 SHALL 据该桶核心末键的手别选用对应偏好表（核心末键可由桶编码经 `last_key = code % code_base - 1` O(1) 还原）。
4. THE 退火出简字按桶内名次（按 `cmp_in_bucket` 排序，0 起）取上屏键 SHALL 满足：每个上屏键 `k` 在该桶被退火占用的次数不超过其剩余容量 `cap_k = max(0, code_num − 该上屏键固定占用数)`，且分配按偏好表顺序「轮次轮转」——第 0 名起依次填第 0 轮（所有 `cap_k>0` 的键按偏好表序各取一次）、第 1 轮（所有 `cap_k>1` 的键）……即名次 i 落在偏好表中第 `⌊i / 当前轮可用键数⌋` 轮、按序的第几个键。被固定简码占满（`cap_k=0`）或被需求 37 异手过滤的上屏键不参与分配。WHERE `code_num=1` 且无固定占用，该规则退化为「名次 i 用偏好表第 `i mod K'` 个上屏键」（与历史轮转一致）。
5. THE 偏好表与手别查找 SHALL 在退火优化前一次性预计算（每级两张小表、一张静态手别表），退火热路径选取上屏键 SHALL 为 O(1) 且不产生堆分配。
6. WHERE 某级 `commit_keys` 为空（K=0），THE 上述偏好与上屏键分配 SHALL 不生效（无上屏键，行为同需求 20.4）。

### 需求 37：上屏键异手过滤（仅出异手上屏键的简码，手感优化）

**用户故事：** 作为方案设计者，我希望每个简码级别能选择「只输出核心末键与上屏键异手的简码」，从而保证简码的最后两键左右手交替、手感更好；同时因为空格键搭配主键盘区任意键手感都好，`_` 作为上屏键应始终允许出简。

#### 术语

- **异手（alternate hand）**：核心简码末键与上屏键分属左/右手不同手（按需求 36 的 QWERTY 手别划分）。空格 `_` 为中性键，视为「与任意手均异手」（始终允许）。

#### 验收标准

1. THE 优化器 SHALL 在每个 `[[simple_levels]]` 级别配置下提供布尔配置项 `commit_alt_hand_only`，表示该级是否仅输出「核心末键与上屏键异手」的简码；缺省为 `false`（不过滤，行为同需求 20/36）。
2. WHERE 某级 `commit_alt_hand_only` 为真，THE 简码评估器为该级某桶选取退火出简上屏键时 SHALL 仅使用「与该桶核心末键异手的字母上屏键」与「空格 `_`（若 `commit_keys` 含之）」，并 SHALL 排除「与核心末键同手的字母上屏键」。
3. WHERE 某级 `commit_alt_hand_only` 为真，THE 空格 `_` SHALL 始终允许出简，不受异手过滤影响（中性键与任意主键搭配手感好）。
4. WHEN 过滤后某桶的可用上屏键序列为 `K'` 项（异手字母数 + 含 `_` ? 1），THE 退火出简字 SHALL 在这 `K'` 个上屏键上按需求 36.4 的「各上屏键剩余容量 `max(0, code_num − occ_k)` 轮次轮转」分配（被固定占满的键 `cap_k=0` 不参与）；WHERE `code_num=1` 且无固定占用，退化为桶内名次 i 用可用序列第 `i mod K'` 个上屏键。
5. WHERE 过滤后某桶的可用上屏键序列为空（`K' = 0`，即无异手字母且无 `_`），THE 该桶 SHALL 不出简，其候选字按既有跨级传播规则上浮到更高级别继续尝试。
6. WHERE 某级 `commit_keys` 为空（K=0）或 `commit_alt_hand_only` 为假，THE 本过滤 SHALL 不生效，行为与需求 20/36 一致。
7. THE 异手过滤 SHALL 仅作用于退火出简的上屏键选择，SHALL NOT 影响固定简码（固定简码为权威字面预分配，即使同手亦原样保留并输出）。
8. THE 异手判定 SHALL 复用需求 36 的预计算偏好表与静态手别表，退火热路径 SHALL 为 O(K) 小集合过滤、不产生堆分配（`commit_keys` 假定小）。
...
2. THE 固定简码的简码字符串 SHALL 为完整的字面击键串，其最后一键可以是任意上屏键（字母或下划线 `_`），也可以没有上屏键（自动上屏级别）。
3. THE 优化器 SHALL 使固定简码映射不影响按 `simple_coverage_ratio` 从全集汉字选取候选字集合的计算，即候选字集合仍以全集汉字按字频覆盖率选取。
4. WHEN 候选字集合选取完成后，THE 优化器 SHALL 从候选字集合中移除已在固定简码映射中预先分配的汉字，使其不参与退火的简码分配（无论该固定简码是否归属到某个级别）。
5. THE 优化器 SHALL 按如下规则确定长度为 T 的固定简码 FC 所属级别（两种解释，任一命中即归属）：
   - 「纯核心」：存在某级 L 的 `commit_keys` 为空且其核心简码键位数等于 T → FC 归属 L，占用该级普通名额（不占 commit slot）。
   - 「核心+上屏」：存在某级 L 的 `commit_keys` 非空、其核心简码键位数等于 T−1、且 FC 的最后一键 ∈ L 的 `commit_keys` → FC 归属 L，并占用该级该核心简码桶里「FC 最后一键」对应的 commit slot。
   IF 同时命中多个级别，THEN THE 优化器 SHALL 选取级别号最小者并告警。
6. WHERE 某固定简码无法命中任何级别（如一码固定简码但所有一键级别均配置了非空 `commit_keys`），THE 优化器 SHALL 仅输出告警且不将其归属任何级别，但 SHALL NOT 丢弃它：该字仍从候选集排除、其常量贡献仍计入简码指标、其 `all_assigned_flags` 仍恒为真（计入重码排除），且仍在 output 文件中输出；该「无归属」固定简码 SHALL NOT 占用任何级别的任何 commit slot。
7. WHEN 简码评估器为某级别的某个核心简码桶选取退火出简候选，THE 简码评估器 SHALL 按「每有效简码至多 `code_num` 个全码字」分配：对该桶的每个有效简码（无上屏键级别为核心简码自身；有上屏键级别为「核心简码 + 各个上屏键」，并先经需求 37 异手过滤剔除被过滤的上屏键），其退火可出简数为 `max(0, code_num − 该有效简码的固定占用数)`；该核心桶退火出简总数 SHALL 为各有效简码可出简数之和，即 `Σ_{有效上屏键 k} max(0, code_num − occ_k)`（无上屏键级别即 `max(0, code_num − occ)`）。退火字使用的上屏键按需求 36.4 在「各上屏键剩余容量」上轮转分配，使任一上屏键被退火占用的次数不超过其剩余容量。固定占用数本身不受 `code_num` 限制（固定简码为权威预分配，同一有效简码上可有 ≥1 个、甚至达到或超过 `code_num`，此时该有效简码退火出 0），且固定简码不因占用达到/超过 `code_num` 而被拒绝。
8. THE 简码评估器 SHALL 将固定简码的汉字视为已出简：其字频计入简码覆盖率、其简码键位（含其自身结尾上屏键）计入加权当量与分布偏差，且在简码重码统计中作为「已出简」从全码桶排除。
9. THE 固定简码的汉字对简码各项指标的贡献 SHALL 为不随分配变化的常量（因固定简码为字面键位串，与字根到键位的分配无关）。
10. WHEN 优化器将简码输出到 output 文件（`output-simple-codes.txt` 与 `output-combined.txt`），THE 优化器 SHALL 输出全部固定简码（保留其字面上屏键）：归属某级者随该级输出，无归属者在所有级别输出之后以单独一段输出，二者均不得遗漏。
11. THE 优化器 SHALL 在配置确认日志中逐条输出每个固定简码所归属的级别号；无归属者输出告警说明其仍被输出与排除候选、但不占任何 commit slot。
12. THE 优化器 SHALL 在 `config.toml.example` 与 `moling/config.toml` 中提供固定简码的注释示例（默认整段注释掉），为「不、是、我、的、了」分别示例简码「u、i、o、e、a」，仅用于表达配置格式。
13. WHERE 某 `[[simple_levels]]` 级别的 `code_num` 为 0，THE 优化器 SHALL 使归属该级的固定简码仍然生效（输出并从候选集排除），且该级退火不分配任何简码；为此 `get_simple_code_config` SHALL 保留「有固定简码按码长归属到其上」的 `code_num=0` 级别，并丢弃无固定简码归属的 `code_num=0` 级别以避免无谓开销。
...
1. THE 简码评估器 SHALL 仅当某候选字在某级别的**核心码长**（指令步数，不含上屏键）严格小于该字全码长度时，才允许该字在该级别出简；全码长度等于 `char_infos[ci].parts.len()`。资格判定不计入上屏键，以避免「全码比简码仅多一键」时非空 `commit_keys` 使有效长度追平全码而被误拒。
2. WHERE 某候选字在某级别的核心码长大于或等于其全码长度，THE 简码评估器 SHALL 不将该字纳入该级别的简码桶，使其在该级别不出简。
3. THE 优化器 SHALL 对固定简码施加同一约束：IF 某固定简码的核心码长（去除结尾上屏键后）大于或等于对应字的全码长度，THEN THE 优化器 SHALL 输出告警并拒绝该条固定简码。
...
3. WHERE 某简码桶被固定简码占用，THE 优化器 SHALL 使该桶每个有效简码的「固定占用 + 优化出简」不超过 `code_num`，即该有效简码的优化出简数不超过 `max(0, code_num − 该有效简码固定占用数)`；该桶优化出简总数不超过 `Σ_{有效上屏键} max(0, code_num − occ)`（无上屏键级别即 `max(0, code_num − occ)`）。
...
10. THE 占用保护判定 SHALL 始终以核心简码桶编码 S（不含上屏键）与受保护汉字全码比较。**备注（已知近似）**：当某级 `commit_keys` 非空时，实际击键码为「S + 上屏键」（比 S 长 1），理论上应以「S+上屏键」与全码比较才精确；本设计有意保持按 S 判定不变（实现简单、且上屏键使实际码更长、撞短全码概率更低），由方案设计者自行保证「为简码配置上屏键后不会与某汉字全码冲突」。WHERE `commit_keys` 为空，THE 占用保护语义 SHALL 与历史完全一致。
...
1. THE 简码评估器在脏桶重选（`reselect_bucket`，每步热路径）选取桶内前 `N` 个出简（`N` = 该桶退火可出简名额，见需求 21.7）时，SHALL 使用部分选择（`select_nth_unstable_by`）而非整桶全排序，复杂度为 O(桶成员数)。
2. THE 部分选择得到的出简集合 SHALL 与整桶全排序后取前 `N` 个**逐元素相同**（`cmp_in_bucket` 为严格全序，故「最优 `N` 个」的集合唯一确定）；当 `N == 0` 或 `N ≥ 桶成员数` 时 SHALL 跳过排序/分划（选中集合与桶内顺序无关）。
